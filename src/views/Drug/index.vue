<template>
  <div class="drug">
    ...
    <el-card>
      ...
      <el-table :data="data.FilteredData" :row-key="(row) => row.id" table-layout="auto" stripe>
        ...
        >
          ...
          <template v-slot="{ row }" v-else-if="item.prop === 'exp'">
            {{ $utils.formatDate(row.exp, 'yyyy-MM-dd') }}
          </template>
          <template v-slot="{ row }" v-else-if="item.prop === 'date'">
          ...
          <template #default="{ row }" v-if="item.prop === 'action'">
            ...
            <el-button type="danger" size="small" @click="submitDeleteForm('药品', row.id)">
              {{ t('table.delete') }}
            </el-button>
          </template>
        </el-table-column>
    ...
    <!-- 对话框 -->
    <DrugDialog v-model="dialogVisible" :dialogTitle="dialogTitle" :dialogTableValue="dialogTableValue" />

    <!-- 抽屉 -->
    <Drawer v-model="drawerVisible" :drawerTitle="t('main.drug.drawerTitle')" :drawerTableValue="drawerTableValue" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const getListData = async () => {
    ...
    const { count, drugs }: { count: number; drugs: Array<Drug> } = await getAllDrug({
      offset: getPageOffset(data.queryForm.currentPage, data.queryForm.pageSize),
      limit: data.queryForm.pageSize,
    })
    data.total = count
    drugsRef.value = drugs
    data.FilteredData = drugs.slice()
  }
  await getListData()
  ...
  const dialogTableValue = ref<Drug>()
  const changeDialogVisible = async (row: Drug | null) => {
    if (isUndef(row)) {
      dialogTitle.value = t('main.drug.insertDrug')
      dialogTableValue.value = undefined
    } else {
      dialogTitle.value = t('main.drug.updateDrug')
  ...
  const changeDrawerVisible = async (row: Drug) => {
  ...
  // 数据筛选
  const searchData = () => {
    data.FilteredData = drugsRef.value.filter(({ id, name, price, quantity, batch, pd, exp, createdAt }) =>
      `${id}${name}${price}${quantity}${batch}${pd}${exp}${createdAt}`.toLowerCase().includes(data.query.toLowerCase())
    )
  }
</script>

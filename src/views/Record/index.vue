<template>
  <div class="record">
    <Title>{{ t('aside.menu.record') }}</Title>
    <el-card>
      <el-row :gutter="20">
        <el-col :span="7">
          <el-input
      ...
      <el-table :data="data.FilteredData" :row-key="(row) => row.id" table-layout="auto" stripe>
        ...
        >
          ...
          <template #default="{ row }" v-if="item.prop === 'action'">
            ...
            <el-button type="danger" size="small" @click="submitDeleteForm('病历', row.id)">
              {{ t('table.delete') }}
            </el-button>
          </template>
        </el-table-column>
    ...
    <!-- 对话框 -->
    <RecordDialog v-model="dialogVisible" :dialogTitle="dialogTitle" :dialogTableValue="dialogTableValue" />

    <!-- 抽屉 -->
    <Drawer
      v-model="drawerVisible"
      :drawerTitle="t('main.record.drawerTitle')"
...
<script lang="ts" setup>
  ...
  const recordsRef = ref<Array<Record>>([])
  ...
  const getListData = async () => {
    ...
    const { count, records }: { count: number; records: Array<Record> } = await getAllRecord({
    ...
    data.FilteredData = records.slice()
  }
  await getListData()
  ...
  const dialogTableValue = ref<Record>()
  const changeDialogVisible = async (row: Record | null) => {
    if (isUndef(row)) {
      dialogTitle.value = t('main.record.insertRecord')
      dialogTableValue.value = undefined
    } else {
      dialogTitle.value = t('main.record.updateRecord')
  ...
  const changeDrawerVisible = async (row: Record) => {
  ...
  // 数据筛选
  const searchData = () => {
    data.FilteredData = recordsRef.value.filter(({ id, name, illness, text, dp, doctor, createdAt }) =>
      `${id}${name}${illness}${text}${dp}${doctor}${createdAt}`.toLowerCase().includes(data.query.toLowerCase())
    )
  }
</script>
